...
const STEPS = [
  ...
  [
    'エフェクトを選ぶ',
    '画面下のナビゲーションでグループ（パターン・ゆがみ・ずれ など）を選び、次に使いたいエフェクトを選びます。項目が多いときは左右にスワイプできます。',
  ],
  [
    '調整する',
    '設定項目のボタンを切り替えながら、スライダーなどで調整します。✓ で確定、✕ で取り消しです。',
  ],
  [
    '重ねる・並べ替える',
    '画像右下のボタンで「重ねたエフェクト」を開くと、並べ替え・一時的に隠す・外すができます。タップするとそのエフェクトの調整に戻れます。',
  ],
  ['保存する', '右上の「保存」で原寸の画像を書き出します。スマホでは「共有」から写真アプリなどに送れます。'],
];

const FAQ = [
  ['画像はどこかに送信されますか？', 'いいえ。すべての処理はお使いのブラウザの中だけで行われ、画像がサーバーに送信されることはありません。'],
  ['元の画像と見比べたい', '画像を長押ししている間、エフェクトをかける前の画像が表示されます。'],
  [
    'エフェクトの順番に意味はありますか？',
    'はい。下にあるエフェクトから順番にかかり、上にあるエフェクトほど後からかかります。「重ねたエフェクト」で ⠿ をドラッグして入れ替えると仕上がりが変わります。',
  ],
  ['同じエフェクトを 2 回かけたい', 'エフェクトの調整画面の右上にある複製ボタンで、同じエフェクトをもう 1 つ重ねられます。'],
  ['「強さ」と「重ね方」とは？', 'どのエフェクトにもある共通の設定です。「強さ」はエフェクトの濃さ、「重ね方」はエフェクト結果を下の画像にどう合成するか（乗算・スクリーンなど）を決めます。'],
  ['「ランダム」とは？', '不揃いさやグリッチなどランダムな要素の出方を変えます。同じ設定なら何度書き出しても同じ結果になります。'],
  ['画像をクリアするとどうなりますか？', '画像が閉じて最初の画面に戻ります。重ねたエフェクトの構成は残るので、別の画像に同じエフェクトをかけられます。'],
